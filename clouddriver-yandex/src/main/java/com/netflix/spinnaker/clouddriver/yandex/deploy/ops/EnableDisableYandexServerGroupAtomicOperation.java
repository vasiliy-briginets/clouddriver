...
public class EnableDisableYandexServerGroupAtomicOperation implements AtomicOperation<Void> {
  ...
  public EnableDisableYandexServerGroupAtomicOperation(EnableDisableYandexServerGroupDescription description, boolean disable) {
  ...
  public Void operate(List priorOutputs) {
    ...
    getTask().updateStatus(phaseName, "Initializing " + verb + " server group operation for " + serverGroupName + "...");

    YandexCloudServerGroup serverGroup = yandexClusterProvider
      .getServerGroup(description.getAccount(), "ru-central1", serverGroupName);

    getTask().updateStatus(phaseName, presentParticipling + " server group " + serverGroupName + "...");
    if (disable) {
      disableInstanceGroup(phaseName, serverGroup);
    } else {
      enableInstanceGroup(phaseName, serverGroup);
    }
    getTask().updateStatus(phaseName, "Done " + presentParticipling.toLowerCase() + " server group " + serverGroupName + ".");
    return null;
  }
  ...
  private void enableInstanceGroup(String phaseName, YandexCloudServerGroup serverGroup) {
    OpsHelper.enableInstanceGroup(
      ...
      serverGroup.getLoadBalancerIntegration() == null ? null : serverGroup.getLoadBalancerIntegration().getTargetGroupId(),
  ...
  private void disableInstanceGroup(String phaseName, YandexCloudServerGroup serverGroup) {
    ...
    serverGroup.getLoadBalancerIntegration().getBalancers().forEach(balancer -> {
      DetachNetworkLoadBalancerTargetGroupRequest request = DetachNetworkLoadBalancerTargetGroupRequest.newBuilder()
        .setNetworkLoadBalancerId(balancer.getId())
        .setTargetGroupId(targetGroupId)
        .build();
      getTask().updateStatus(phaseName, "Deregistering server group from load balancer " + balancer.getName() + "...");
      Operation operation = description.getCredentials().networkLoadBalancerService().detachTargetGroup(request);
      operationPoller.waitDone(description.getCredentials(), operation, phaseName);
      getTask().updateStatus(phaseName, "Done deregistering server group from load balancer " + balancer.getName() + ".");
    });
  }
