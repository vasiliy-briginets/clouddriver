...
public class DestroyYandexServerGroupAtomicOperation implements AtomicOperation<Void> {
  ...
  public Void operate(List priorOutputs) {
    ...
    getTask().updateStatus(BASE_PHASE, "Initializing destruction of server group " + serverGroupName + "... ");

    YandexCloudServerGroup serverGroup = yandexClusterProvider.getServerGroup(description.getAccount(), "ru-central1", serverGroupName);
  ...
  public void detachFromLoadBalancers(YandexCloudServerGroup.LoadBalancerIntegration loadBalancerIntegration) {
    ...
    loadBalancerIntegration.getBalancers().stream()
      .map(b -> DetachNetworkLoadBalancerTargetGroupRequest.newBuilder()
        .setNetworkLoadBalancerId(b.getId())
        .setTargetGroupId(loadBalancerIntegration.getTargetGroupId())
        .build())
      .map(request -> description.getCredentials().networkLoadBalancerService().detachTargetGroup(request))
      .forEach(operation -> operationPoller.waitDone(description.getCredentials(), operation, BASE_PHASE));
